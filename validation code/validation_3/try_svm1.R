...
gpf<-read.table("gene33_fc_p_limma.txt",header = TRUE,sep="\t")

or<-gpf[order(gpf$P.Value,decreasing=F),]


data<-data.frame(or[5:33,1])
...
#Feature_Gene<-read.table(file=paste("D:\\A_My_Data\\HCC\\1_data\\validation_3\\GenesSelect_byRFE\\",Clu_FileNames[j],sep=""),header = T)
...
roc_clu <- plot.roc(as.numeric(feature_gene_expr_label$if_health),as.numeric(fitted),
                    main="GSE64041 Data", percent=TRUE, col="blue",print.auc=T)
...
roc_clu <- plot.roc(as.numeric(feature_gene_expr_label_2$if_health),as.numeric(fitted_2),
                    percent=TRUE, col=rainbow(60)[20],main="GSE45436 Data",print.auc=T)#
...
roc_clu <- plot.roc(as.numeric(rna_data$if_health),as.numeric(fitted_3),
                    percent=TRUE, col=rainbow(60)[40],main="RNA_Seq Data",print.auc=T)#
...
all_auc<-read.table("t_test_AUC.txt",header=T,sep="\t")
row.names(all_auc)<-all_auc$GEO

r <- barplot(all_auc$clu3_AUC, col = rainbow(20))
...
write.table(test_fitted,"v3_test_fitted.txt",quote=F,sep="\t")
write.table(test_if_health,"v3_test_if_health.txt",quote=F,sep="\t")
write.table(test_fitted_2,"v3_test_fitted_2.txt",quote=F,sep="\t")
write.table(test_if_health_2,"v3_test_if_health_2.txt",quote=F,sep="\t")
write.table(test_fitted_3,"v3_test_fitted_3.txt",quote=F,sep="\t")
write.table(test_if_health_3,"v3_test_if_health_3.txt",quote=F,sep="\t")
