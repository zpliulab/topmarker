...
diffGene_p<-merge(diffGene2,genename_pvalue_fdr,by.x="gene_select",by.y="genename",all=FALSE)
#diffGene_order<-diffGene_p[order(diffGene_p$fdr),]
diffGene_order<-diffGene_p[order(diffGene_p$P.Value),]
...
#diffGene_order_p<-diffGene_order[diffGene_order$fdr<0.01,]
diffGene_order_p<-diffGene_order[diffGene_order$P.Value<0.01,]
...
for(j in 1:50){#
  ...
  Random_Gene <- data.frame(diffGene_order_p[ sample(3001:4000,33,replace = F),1 ])#nrow(diffGene_order_p)
...
#write.table(mean_auc,"mean_auc_p_4001_5931.txt",quote=FALSE,sep="\t")
write.table(mean_auc,"limma_mean_auc_p_3001_4000.txt",quote=FALSE,sep="\t")
