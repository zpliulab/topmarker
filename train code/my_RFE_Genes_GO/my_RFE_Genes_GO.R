setwd("C:\\Users\\Yanqiu Wang\\Desktop\\HCC\\3_network\\my_clusterProfiler\\my_RFE_Genes_GO")
library(tidyverse)
library(httr)
library(jsonlite)
...
Gene_RFE_Path <- "C:\\Users\\Yanqiu Wang\\Desktop\\HCC\\3_network\\my_clusterProfiler\\my_RFE_Genes_GO\\GenesSelect_byRFE"
RFE_FileNames <- dir(Gene_RFE_Path)
######  
#对每个社群进行GO分析并画图。  
for(i in 1:(length(RFE_FileNames))){#
  data <- read.table(file = paste(RFE_FileNames[i],sep = "\t"),header = T)
  ...
  write.table(go_enrich_df,file=paste("GO",RFE_FileNames[i],sep=""),quote = F,sep='\t',row.names = F)
  ...
  p <- ggplot(data=go_enrich_df, aes(x=number, y=GeneNumber,fill=type)) +
    ...
    labs(title = paste("The Most Enriched GO Terms of",RFE_FileNames[i],sep=" "))
  
  pdf(file=paste("GO_enrichment_",RFE_FileNames[i],".pdf",sep=""))
  plot(p)
  dev.off()
...
#####
#对每个社群进行kegg分析并画图。  
for(j in 1:(length(RFE_FileNames))){#
  data <- read.table(file = paste(RFE_FileNames[j],sep="\t"),header = T)
  gene_id_sym <- merge(data,homo,by.x="gene_select",by.y="Symbol",all=FALSE)
  gene_id<-gene_id_sym[,2]#取出每个社群中包含的基因名，匹配成基因ID，变成逗号相连的list。
  ...
  write.table(keggf,file=paste("kegg_",RFE_FileNames[j],sep=''),quote = F,sep='\t',row.names = F)
  
  
  kk<-dotplot(kegg,title=paste("kegg enrichment of ",RFE_FileNames[j],sep=''))
  pdf(file=paste("kegg_",RFE_FileNames[j],".pdf",sep=''))
  plot(kk)
  dev.off()
  
  # ee<-emapplot(kegg,title=paste("kegg enrichment of ",RFE_FileNames[j],sep='', showCategory = 30))
  # pdf(file=paste("kegg_ema_",RFE_FileNames[j],".pdf",sep=''))
  # plot(ee)
  # dev.off()
