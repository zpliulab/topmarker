...
ppi<-as.matrix(read.table("nont_cor_2gene.txt",sep="\t",header = T))
...
write.table(p_igraph,"n_igraph.txt",quote = FALSE,sep = "\t")
...
write.table(p_sna,"n_sna_3.txt",quote = FALSE,sep = "\t")
...
write.table(p_centiserve,"n_centiserve_15.txt",quote = FALSE,sep = "\t",row.names = TRUE)
